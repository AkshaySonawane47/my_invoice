...
from .forms import InvoiceUpdateform, LineItemFormset, InvoiceForm
...
def createInvoice(request):
    ...
    if request.method == 'GET':
        ...
        if form.is_valid():
            invoice = Invoice.objects.create(student_name=form.data["student_name"],
...
def update_invoice(request, id):
        ...
        form= InvoiceUpdateform(queryset)
        if request.method == 'POST':
            form= InvoiceUpdateform(request.POST,queryset)
            # if form.is_valid():
            form.save()
            return redirect('invoice:invoice-list')
